...
function Carousel() {
    ...
    this.setSlides = function (SLIDES) {
        for (var i = 0; i < SLIDES.length; i++) {
            var item = this.createCarouselItem();
    ...
    this.setIndicators = function () {
        for (var i = 0; i < Math.round(this.slideCount / 4); i++) {
...
function request(car) {
    ...
    for (var i = 0; i < car.allCarouselImage.length; i++) {
    ...
    if (str != "") {
        ...
        if (x.status != 200) {
            ...
            var slides = JSON.parse(x.responseText);
